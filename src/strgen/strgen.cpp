...
struct FileWriter {
	...
	{
		output_stream.open(path, openmode);
	}
...
struct HeaderFileWriter : HeaderWriter, FileWriter {
	...
	{
		output_stream << "/* This file is automatically generated. Do not modify */\n\n";
		output_stream << "#ifndef TABLE_STRINGS_H\n";
		output_stream << "#define TABLE_STRINGS_H\n";
	}

	void WriteStringID(const std::string &name, int stringid)
	{
		if (prev + 1 != stringid) output_stream << "\n";
		output_stream << fmt::format("static const StringID {} = 0x{:X};\n", name, stringid);
		prev = stringid;
		total_strings++;
	}
	...
	void Finalise(const StringData &data)
	{
		...
		output_stream << "#endif /* TABLE_STRINGS_H */\n";
...
#	endif
			...
			if (error_code) FatalError("rename({}, {}) failed: {}", this->path.generic_string(), this->real_path.generic_string(), error_code.message());
		}
	}
};
...
struct LanguageFileWriter : LanguageWriter, FileWriter {
	...
	{
		output_stream.write((const char *)buffer, length);
	}
};
	...
	try {
		...
		if (mgo.numleft == 0) {
			...
			for (int i = 0; i < mgo.numleft; i++) {
				...
				if ((_show_todo & 2) != 0) {
					fmt::print("{} warnings and {} errors for {}\n", _warnings, _errors, output_file.generic_string());
				}
			}
		}
	} catch (...) {
