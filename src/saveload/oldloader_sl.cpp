...
static bool LoadOldGood(LoadgameState *ls, int num)
{
	...
	if (GB(_waiting_acceptance, 0, 12) != 0 && CargoPacket::CanAllocateItem()) {
		ge->GetOrCreateData().cargo.Append(new CargoPacket(GB(_waiting_acceptance, 0, 12), _cargo_periods, (_cargo_source == 0xFF) ? INVALID_STATION : static_cast<StationID>(_cargo_source), INVALID_TILE, 0),
				INVALID_STATION);
	}
...
static bool LoadOldStation(LoadgameState *ls, int num)
{
	...
	_current_station_id = static_cast<StationID>(num);
...
bool LoadOldVehicle(LoadgameState *ls, int num)
{
	...
	for (uint i = 0; i < _old_vehicle_multiplier; i++) {
		...
		if (_cargo_count != 0 && CargoPacket::CanAllocateItem()) {
			StationID source =    (_cargo_source == 0xFF) ? INVALID_STATION : static_cast<StationID>(_cargo_source);
