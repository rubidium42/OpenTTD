...
struct NewGRFParametersWindow : public Window {
	...
	int clicked_button;    ///< The row in which a button was clicked or INT_MAX when none is selected.
	bool clicked_increase; ///< True if the increase button was clicked, false for the decrease button.
	bool clicked_dropdown; ///< Whether the dropdown is open.
	bool closing_dropdown; ///< True, if the dropdown list is currently closing.
	int clicked_row;       ///< The selected parameter, or INT_MAX when none is selected.
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			case WID_NP_BACKGROUND: {
				...
				int num = this->vscroll->GetScrolledRowFromWidget(pt.y, this, WID_NP_BACKGROUND);
	...
	void OnInvalidateData([[maybe_unused]] int data = 0, [[maybe_unused]] bool gui_scope = true) override
	{
		...
		if (this->clicked_row != INT_MAX && this->clicked_row >= this->vscroll->GetCount()) {
			this->clicked_row = INT_MAX;
			this->CloseChildWindows(WC_QUERY_STRING);
		}
	}
